...
int main(){
    ...
    while(choice){
    	...
    	if (choice > 0 && choice <= 17) system("clear");
    	...
    	switch(choice){
    		...
    		case 10: // Indicar colega diferente
    			...
    			if (p != NULL){
    				...
    				sugerir_diferente(&Alunos->raiz, p, &q, &comum);

					if (q != NULL){
						int qtd = filmes_fav_comum(p, p);

						if ((comum != MAX_INT && comum != qtd) || qtd == 0){
							...
							if (comum != 0)
								printf("} (apenas %d em comum!)", comum);
							else
